...
<div class="bg-gray-100 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-4 sm:mb-6 text-center">Buku SMP</h1>
                    ...
                    <div class="p-4 sm:p-5">
                        ...
                        <div class="relative">
                            <select onchange="window.location.href=this.value" 
                                ...
                                <option value="{{ route('category.show', ['slug' => 'smp']) }}" {{ !request('grade') ? 'selected' : '' }}>Semua Kelas</option>
                                <option value="{{ route('category.show', ['slug' => 'smp', 'grade' => 7]) }}" {{ request('grade') == 7 ? 'selected' : '' }}>Kelas 7</option>
                                <option value="{{ route('category.show', ['slug' => 'smp', 'grade' => 8]) }}" {{ request('grade') == 8 ? 'selected' : '' }}>Kelas 8</option>
                                <option value="{{ route('category.show', ['slug' => 'smp', 'grade' => 9]) }}" {{ request('grade') == 9 ? 'selected' : '' }}>Kelas 9</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm">
                    <div class="p-4 sm:p-5">
                        ...
                        <div class="relative">
                            <select onchange="window.location.href=this.value" 
                                    class="block w-full bg-white border border-gray-300 hover:border-blue-500 px-3 sm:px-4 py-2 sm:py-3 rounded-lg text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                                <option value="{{ route('category.show', ['slug' => 'smp']) }}" {{ !request('subject') ? 'selected' : '' }}>Semua Pelajaran</option>
                                @foreach(App\Models\Book::getSubjects() as $value => $label)
                                    <option value="{{ route('category.show', ['slug' => 'smp', 'subject' => $value]) }}" {{ request('subject') == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <div class="flex-1 bg-white rounded-lg shadow-sm overflow-hidden">
                <div class="p-4 sm:p-6">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mb-4">
                                ...
                                <p class="text-sm text-gray-600 mb-3">
                                    <span class="font-medium">Kurikulum:</span> {{ $book->curriculum ?? 'Kurikulum Merdeka' }}
                                </p>
                    ...
                    <div class="text-center py-8 sm:py-10">
                        <p class="text-gray-500 text-sm sm:text-base">Tidak ada buku SMP yang tersedia saat ini.</p>
                    </div>
                    @endforelse
                </div>
            </div>
